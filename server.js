...
async function creditIncomingGift(message) {
    ...
    // ВАЖНО: business_message приходит на ЛЮБОЕ сообщение в чатах,
    // подключённых к business-аккаунту — включая сообщения, которые
    // ОТПРАВЛЯЕТ сам этот аккаунт (например, когда мы вручную/через
    // юзербота дарим подарок кому-то ИЗ хранилища). Без этой проверки такая
    // исходящая отправка ошибочно принималась за новый входящий депозит и
    // задваивала подарок в инвентаре — именно это чинит проверка ниже.
    const connection = getActiveBusinessConnection();
    if (connection && String(sender.id) === String(connection.business_account_tg_id)) {
        console.log(`ℹ️  Это исходящее сообщение от самого хранилища (${sender.id}), не депозит — пропускаю.`);
        return;
    }
